...
CACHE_LOG=$( (docker exec clab-dns-bc-cache1 cat /var/log/named.log 2>/dev/null; docker exec clab-dns-bc-cache2 cat /var/log/named.log 2>/dev/null) | grep -E "query failed|timed out|SERVFAIL" | tail -n 10 || true)
echo "${CACHE_LOG}"
